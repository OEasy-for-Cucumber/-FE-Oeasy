...
function Chat() {
  ...
  useEffect(() => {
    ...
    const stompClient = new Client({
      ...
      //webSocketFactory: () => new SockJS("http://localhost:8080/ws"), // Spring Boot 서버의 WebSocket 엔드포인트
      reconnectDelay: 5000,
    ...
    stompClient.onConnect = () => {
      ...
      // 메시지 구독
      stompClient.subscribe("/topic/like-votes", (msg) => {
    ...
    stompClient.onConnect = () => {
      // 메시지 구독
      stompClient.subscribe("/topic/hate-votes", (msg) => {
        console.log(msg.body);
      });
      setClient(stompClient);
    };
  ...
  const handleSendMessage = () => {
    ...
    if (client != null) {
      message.trim();
      client.publish({
        destination: "/app/message", // Spring Boot의 @MessageMapping("/message") 경로
        body: JSON.stringify({
          content: message,
          userPk: 38
        })
